...
IREEList::~IREEList() { release(); }

Error IREEList::append(IREETensor &p_tensor) {
	if (!p_tensor.is_captured())
...
void IREEList::release() {
	if (vm_list != nullptr) {
		iree_vm_list_release(vm_list);
		vm_list = nullptr;
	}
}

iree_vm_list_t *IREEList::borrow_vm_list() const { return vm_list; }
...
Array IREEList::get_tensors() const {
	...
	for (iree_host_size_t i = 0; i < size; i++) {
		iree_hal_buffer_view_t *buffer_view =
				iree_vm_list_get_buffer_view_retain(vm_list, i);
