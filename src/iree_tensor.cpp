...
void IREETensor::_bind_methods() {
    ClassDB::bind_static_method("IREETensor", D_METHOD("from_bytes", "bytes", "dimension"), &IREETensor::from_bytes);
    ClassDB::bind_static_method("IREETensor", D_METHOD("from_float32s", "float32s", "dimension"), &IREETensor::from_float32s);

    ClassDB::bind_method(D_METHOD("capture_bytes", "bytes", "dimension"), &IREETensor::capture_bytes);
    ClassDB::bind_method(D_METHOD("capture_float32s", "float32s", "dimension"), &IREETensor::capture_float32s);
...
Ref<IREETensor> IREETensor::from_bytes(PackedByteArray p_data, PackedInt64Array p_dimension) {
    Ref<IREETensor> tensor_ref;
    tensor_ref.instantiate();
    tensor_ref->capture_bytes(p_data, p_dimension);
    return tensor_ref;
}

Ref<IREETensor> IREETensor::from_float32s(PackedFloat32Array p_data, PackedInt64Array p_dimension) {
    Ref<IREETensor> tensor_ref;
    tensor_ref.instantiate();
    tensor_ref->capture_float32s(p_data, p_dimension);
    return tensor_ref;
}
...
Error IREETensor::capture_bytes(PackedByteArray p_data, PackedInt64Array p_dimension) {
    release();
    ...
    ERR_FAIL_COND_V_MSG(
        iree_hal_buffer_view_allocate_buffer(
            iree_hal_device_allocator(IREEInstance::borrow_singleton()->borrow_hal_device()), shape_rank, shape, IREE_HAL_ELEMENT_TYPE_UINT_8, IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
            (iree_hal_buffer_params_t){
                .usage = IREE_HAL_BUFFER_USAGE_DEFAULT,
                .type = IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL
            },
            iree_make_const_byte_span(p_data.ptr(), p_data.size()), &buffer_view
    ), ERR_CANT_CREATE, "Unable to allocate buffer on device for IREE tensor.");
    return OK;
}

Error IREETensor::capture_float32s(PackedFloat32Array p_data, PackedInt64Array p_dimension) {
    release();
    const int64_t shape_rank = p_dimension.size();
    ERR_FAIL_COND_V_MSG(shape_rank == 0, ERR_PARAMETER_RANGE_ERROR, "Dimensionless tensor is forbidden.");
    ERR_FAIL_COND_V_MSG(shape_rank > MAX_SHAPE_RANK, ERR_PARAMETER_RANGE_ERROR, "Dimension exceed maximum rank (" STRINGIFY_MACROS(MAX_SHAPE_RANK) ").");
    ...
    ERR_FAIL_COND_V_MSG(
        iree_hal_buffer_view_allocate_buffer(
            iree_hal_device_allocator(IREEInstance::borrow_singleton()->borrow_hal_device()), shape_rank, shape, IREE_HAL_ELEMENT_TYPE_FLOAT_32, IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR,
            (iree_hal_buffer_params_t){
                .usage = IREE_HAL_BUFFER_USAGE_DEFAULT,
                .type = IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL
            },
            iree_make_const_byte_span(p_data.ptr(), p_data.size() * 4), &buffer_view
    ), ERR_CANT_CREATE, "Unable to allocate buffer on device for IREE tensor.");
    return OK;
}
